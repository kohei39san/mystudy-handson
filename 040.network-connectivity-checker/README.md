...
- `ElasticLoadBalancingReadOnlyAccess`（EC2 チェック時の ELBv2 逆引きに必要）
...
| `public_subnet=true` かつ `public_ip_assigned=true` | ① として必須 | 不要 |
| internet-facing ELBv2 に登録済み (`lb_internet_facing=true`) | ① の代替として必須 | 不要 |
| `private_ip` あり | 不要 | 必須 |

> `internet_reachability=reachable` の条件: `instance_state=running` かつ（① または internet-facing ELBv2 登録済み）
...
| Azure LB バックエンドプール登録済み | 参考情報 | 参考情報 |
