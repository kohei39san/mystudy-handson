...
- ラベルを削除する場合は `safe-outputs.remove-labels.allowed` に定義されたラベルのみを使用する。
- 影響範囲カテゴリから最大1つ、バージョンカテゴリから最大1つを付与する（合計最大2ラベル）。
- 例外として、dependabot の PR ではバージョンカテゴリのラベルのみを付与する（影響範囲カテゴリは付与しない）。
- 再現率より適合率を優先する。
- 新しいラベルは作成しない。
- 既存ラベルが今回の判定と矛盾する場合は、safe-outputs の上限内で削除を検討する。
...
6. 各カテゴリで、既存の管理対象ラベルが今回の選定ラベルと異なる場合は削除候補にする。
7. 確信度が低い場合は、ラベルの追加・削除を行わない。
...
7. 選定ラベルと既存の管理対象ラベルを比較し、`labels_to_remove` を決定する。
8. ラベル追加および/または削除の safe output を出力する。
9. どのラベルを追加/削除したかとその理由を日本語で PR コメントとして投稿する。
...
- `labels_to_remove` は `safe-outputs.remove-labels.allowed` に含まれるラベルのみとする。
- 各カテゴリで最大1ラベルとする。
