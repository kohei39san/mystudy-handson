...
- 構成図（drawioファイル、svgファイルなど）は機能ディレクトリ/srcディレクトリに格納すること
  - 標準的な配置: `機能ディレクトリ/src/architecture.drawio` および `機能ディレクトリ/src/architecture.svg`（自動生成）
  - README.mdからは `![アーキテクチャ図](src/architecture.svg)` で参照すること
