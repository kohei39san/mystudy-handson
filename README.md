...
このリポジトリのプルリクエストには、以下のカテゴリのラベルが自動付与されます。

### 影響範囲ラベル（Impact scope）

変更の種類・影響範囲を表します（最大1個）。

| ラベル | 説明 |
|--------|------|
| `feature` | 新機能の追加 |
| `fix` | 既存機能の修正（仕様調整・軽微な不具合修正を含む） |
| `bug` | ユーザーが確認できる再現可能な不具合の修正 |

### バージョンラベル（Version）

セマンティックバージョニングにおけるバージョン変更の種類を表します（最大1個）。
...
| ラベル | 説明 |
|--------|------|
| `dependencies` | 依存関係の更新（Dependabotなど） |

### ラベルの自動付与

プルリクエストが作成・更新されると、`agentic-pr-auto-labeler` ワークフローが自動的にラベルを判定して付与します。
付与したラベルとその根拠は、プルリクエストのコメントに日本語で記載されます。
