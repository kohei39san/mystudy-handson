...
プルリクエストには以下のラベルが自動または手動で付与されます。ラベルは **影響範囲** と **バージョン** の2カテゴリに分類され、それぞれ最大1つが付与されます。

### 影響範囲 (Impact Scope)

変更内容の種類を表します。以下から最大1つを付与します。
...
セマンティックバージョニングに基づくバージョンへの影響度を表します。以下から最大1つを付与します。

| ラベル | 説明 |
|--------|------|
| `major` | 後方互換性のない破壊的変更 |
| `minor` | 後方互換性のある新機能の追加 |
| `patch` | 後方互換性のあるバグ修正や小規模な修正 |

ラベルはAgenticワークフロー（`agentic-pr-auto-labeler`）によって自動付与され、付与根拠が日本語でPRにコメントされます。
