...
### 1. インストール
```bash
# 依存関係をインストール（リポジトリルートの統合requirements.txtを使用）
...
```json
{
  "mcpServers": {
    "redmine": {
      "command": "python",
      "args": ["C:\\path\\to\\mystudy-handson\\034.redmine-mcp-server\\src\\redmine_mcp_server.py"],
      "env": {
...
- **プロンプト**: `@redmine-overdue-tickets`
- **プロンプトファイル**: `.amazonq\prompts\redmine-overdue-tickets.md`

自身の期限切れチケットを自動検索し、優先度順で表示します。
...
現在、あなたが担当する期限切れの未完了チケットはありません。
```

### 2. 関連チケット横断検索・分析

- **プロンプト**: `@redmine-related-search
テーマ: 「認証機能の改善」`
- **プロンプトファイル**: `.amazonq\prompts\redmine-related-search.md`

テーマを指定すると、AIが関連キーワードを生成して横断検索し、分析結果を提供します。
...
| キーワード | 件数 | 主要チケット |
|------------|------|--------------|
| 認証 | 2件 | #122 セキュリティ監査と脆弱性対応 |
| セキュリティ | 2件 | #122 セキュリティ監査と脆弱性対応 |
