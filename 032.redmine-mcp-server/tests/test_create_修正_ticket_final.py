#!/usr/bin/env python3
"""
Final test script to create a ticket using 修正 tracker with parent ticket, assignee, and reporter
"""
...
def test_create_修正_ticket_final():
    ...
    try:
        ...
        # Prepare kwargs for create_issue method
        create_kwargs = {
            'issue_subject': '修正チケットのテスト - バグ修正 (Final)',
        ...
        if parent_id:
            create_kwargs['issue_parent_issue_id'] = parent_id
        ...
        if create_result['success']:
            ...
            print(f"  Subject: 修正チケットのテスト - バグ修正 (Final)")
            print(f"  Tracker: 修正")
            ...
            if details_result['success']:
                ...
                print(f"  Assignee: {issue.get('担当者', 'Not assigned')}")
                if issue.get('parent'):
                    print(f"  Parent: {issue['parent']}")
                
                # Look for 指摘者 field
                if '指摘者' in issue:
                    print(f"  指摘者: {issue['指摘者']}")
                
                # Show all custom fields for debugging
                print("\\n  All fields:")
                for key, value in issue.items():
                    if value and key not in ['id', 'subject', 'tracker', 'status']:
                        print(f"    {key}: {value}")
            else:
...
if __name__ == "__main__":
    print("=== Final Test: Create 修正 Ticket with Parent, Assignee, and Reporter ===")
    success = test_create_修正_ticket_final()
