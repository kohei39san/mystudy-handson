...
def main():
    ...
    # Create ticket with 修正 tracker (ID: 4)
    print("Creating ticket with 修正 tracker...")
    
    # Fields for 修正 tracker:
    # - issue_tracker_id: 4 (修正)
    # - issue_subject: Subject
    # - issue_description: Description  
    # - issue_assigned_to_id: Assignee (担当者)
    # - issue_parent_issue_id: Parent issue (親チケット)
    # - issue_custom_field_values_3: Reporter (指摘者)
    
    create_result = redmine.create_issue(
        project_id="hoge-project",
        subject="修正トラッカーテストチケット - 指摘者・担当者・親チケット指定",
        ...
        issue_parent_issue_id="118",  # Parent issue (親チケット)
        issue_custom_field_values_3="1"  # Admin as reporter (指摘者) - using correct field ID
    )
    ...
    if create_result['success']:
        ...
        print(f"Full result: {create_result}")
