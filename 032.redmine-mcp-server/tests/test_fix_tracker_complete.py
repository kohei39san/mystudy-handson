...
def test_fix_tracker_complete():
    ...
    # Use first member as assignee and reporter
    assignee_id = members[0]['id'] if members else '1'
    
    # Create issue with 修正 tracker (ID: 4) and all specified fields
    issue_data = {
        'issue_tracker_id': '4',  # 修正トラッカー
        'issue_subject': 'テスト修正チケット - 完全版',
        'issue_description': '修正トラッカーでの全フィールド指定テスト',
        'issue_parent_issue_id': '120',  # 親チケット
        'issue_assigned_to_id': assignee_id,  # 担当者
        'issue_custom_field_values_3': assignee_id,  # 指摘者（カスタムフィールド）
        'issue_priority_id': '3',  # 高
    ...
    if result['success']:
        ...
        if details_result['success']:
            ...
            for key, value in issue.items():
                if key not in ['id', 'subject', 'tracker', 'status', 'priority']:
                    print(f"  {key}: {value}")
    else:
